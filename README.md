...
7. **Cancelación** (misma pantalla): "Solicitar cancelación" (`canRequestCancellation`, el creador con el documento `SIGNED`) llama `PATCH /api/v1/document/:id/submit-for-cancellation`; "Confirmar cancelación" (`canConfirmCancellation`, cualquier firmante con el documento en `CANCELLATION_PENDING`) llama `PATCH /api/v1/document/:id/confirm-cancellation`. Ambos usan `CancellationConfirmDialog` (no `window.confirm`).
