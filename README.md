...
- **Falta uno de los dos** → formulario para completar el que falta (`PATCH /api/v1/signature/:id`) + opción de eliminar el existente.
- **Ambos completos** → solo visualización + opción de eliminar cada archivo (`DELETE /api/v1/signature/:id/official-file` o `/signature-image`).
...
Al enviar, `useCreateDocumentSignatures` hace **una sola llamada multipart**: `POST /api/v1/documents/signatures` (archivo + `documentData` + `collaborators` + `requiresDifferentSignatures`, todo serializado como JSON dentro de campos de texto) — ya no son dos llamadas encadenadas (`POST /api/v1/document` + `submit-for-authorization`). Al terminar, invalida `['myDocuments']` y limpia el formulario y el widget de carga.
...
1. `DocumentViewSection` carga el detalle (`GET /api/v1/document/:id`): PDF (`secureUrl`), lista de colaboradores con su estado, y los flags `canSign`/`canReject`/`canRequestCancellation`/`canConfirmCancellation`/`myRole`/`myStatus` que calcula el backend según el turno y rol del usuario.
...
3. Si el documento exige verificación (`requiresVerification`/2FA), `useRequestVerificationCode`/`useVerifyCode` corren un paso previo (`POST /api/v1/document/:id/verification-codes` + `/verify`) antes de habilitar la firma.
4. Si `canSign` es `true`, el botón **"Continuar a firmar"** llama `PATCH /api/v1/document/:id/sign` — la firma visual se compone en el backend a partir de la credencial guardada en el paso 1 y la posición elegida en el paso 2.
5. "Rechazar documento" abre un textarea de motivo (mínimo 5 caracteres) y llama `PATCH /api/v1/document/:id/reject`.
6. Si no es el turno del usuario, o ya actuó, se muestra un mensaje contextual en vez de los botones de acción.
7. **Cancelación** (misma pantalla): "Solicitar cancelación" (`canRequestCancellation`, el creador con el documento `SIGNED`) llama `PATCH /api/v1/document/:id/submit-for-cancellation` — **oculto por ahora**: `IS_CANCELLATION_REQUEST_ENABLED = false` en `DocumentView.tsx` impide dibujarlo aunque el backend lo permita; "Confirmar cancelación" (`canConfirmCancellation`, cualquier firmante con el documento en `CANCELLATION_PENDING`) llama `PATCH /api/v1/document/:id/confirm-cancellation`. Ambos usan `CancellationConfirmDialog` (no `window.confirm`).
...
- **`/access-document`**: entry point del link que llega por correo a un colaborador invitado solo por email (sin `userId`/cuenta todavía). Guarda el contexto (`docId`/`collabId`/`email`) en `localStorage` (`lib/pending-signature-context.ts`) y, si hay sesión activa, llama `PATCH /api/v1/document/:id/link-collaborator` para vincular al colaborador con la cuenta ya logueada; si no hay sesión, redirige a `/login` (o registro) y retoma el contexto guardado después.
- **`/public/documents/[id]`** (route group `(public)`, fuera del middleware de auth): **vista pública de verificación** del documento vía `GET /api/v1/document/public/:id`, sin JWT. Qué se muestra lo decide `isCompleted`, que resuelve el backend — la UI nunca decide por su cuenta si hay evidencia que publicar:
...
| Tipo de firma | `Simple` o `E.Firma` (`signatureType` del listado). Un guion cuando el documento no lo tiene registrado — los creados por el endpoint antiguo `POST /api/v1/document` nunca asignaron tipo, y suponer "Simple" sería inventar |
...
`POST /api/v1/auth/login` guarda el JWT en cookie (`setAuthToken`) y redirige a `/dashboard/documents/create` — la ruta por defecto del dashboard (`/dashboard` a secas hace `redirect()` a esta misma ruta). El store (`useAuthStore`) **no** se llena en ese momento — `AuthProvider` (`app/dashboard/_components/AuthProvider.tsx`, envuelve todo el prefijo `/dashboard`, ver `app/dashboard/layout.tsx`) es quien lo hidrata al montar, leyendo `GET /api/v1/users/me` (perfil cacheado en Redis por CURP) y `GET /api/v1/accounts/me` (catálogo de cuentas). Si es la primera vez que el usuario entra (no hay `activeAccount` persistido) — o si el `activeAccount` persistido ya no aparece en el catálogo fresco (acceso revocado, organización eliminada) —, `AuthProvider` cae automáticamente a la cuenta de tipo `PERSONAL` del catálogo.

**Registro con OTP** (`/signup` → `/signup/verify`): a diferencia de lo anterior, el registro ya no deja al usuario autenticado de inmediato — `POST /api/v1/auth/register` crea una pre-cuenta (`isEmailVerified: false`) y envía un código; `/signup/verify` (`VerifyOtpForm`) lo confirma y recién ahí autentica.

**CAPTCHA en el registro (Cloudflare Turnstile)**: `SignupForm` monta `TurnstileWidget` (`components/form/turnstile-widget.tsx`) y no envía nada hasta tener su token, que viaja como `turnstileToken` dentro del cuerpo de `POST /api/v1/auth/register`; el backend lo verifica contra Siteverify **antes** de crear el pre-registro. La clave del sitio la lee `app/signup/page.tsx` en el servidor (`process.env.TURNSTILE_SITE_KEY`, sin prefijo `NEXT_PUBLIC_` a propósito: así no queda incrustada en el bundle y la misma imagen sirve para todos los entornos) y la pasa como prop al formulario. En Docker esa variable se inyecta **al arrancar** el contenedor (`docker run -e TURNSTILE_SITE_KEY=...` o `environment:` en compose): está declarada como `ENV` en el stage de runtime del `Dockerfile` y no como `ARG` del build, y el `.dockerignore` excluye los `.env*`, así que la clave nunca queda dentro de la imagen. Si el contenedor arranca sin ella, `/signup` muestra el aviso del widget y no deja registrar. La clave secreta nunca toca el frontend, solo `signature-server`. Como el token es de un solo uso, cualquier error del registro reinicia el widget (`useRegister({ onError })` → `TurnstileWidgetHandle.reset()`); lo mismo cuando el reto expira solo. Todo fallo del CAPTCHA —script bloqueado, clave rechazada para el dominio, o `error-callback` de Cloudflare— termina en un aviso con botón **Reintentar verificación** que reinicia el reto (o vuelve a montar el widget si nunca llegó a existir) sin recargar la página: antes el usuario se quedaba con el widget muerto y el botón de envío pidiéndole "completa la verificación" sin nada que completar. El render explícito se hace llamando a `turnstile.render()` en el `onload` del script y **no** a través de `turnstile.ready()`: ese helper solo sirve para encolar trabajo antes de que cargue `api.js` y, con el script ya cargado, Cloudflare descarta el callback. **Recuperación de contraseña** (`/forgot-password`, `ForgotPasswordWizard`): wizard de 3 pasos sin cambiar de URL (email → OTP → nueva contraseña), contra `POST /api/v1/auth/forgot-password` / `/api/v1/auth/verify-reset-code` / `/api/v1/auth/reset-password`.
...
Cookie `token` (1 día, `sameSite: 'lax'`, `secure` solo en producción). `logout()` llama `POST /api/v1/auth/logout` y siempre limpia la cookie localmente en el `finally`, incluso si la llamada falla.
...
| Función | Endpoint |
|---|---|
| `getCurrentUserRequest` | `GET /api/v1/auth/me` (perfil completo, incluye URLs prefirmadas de firma/INE; usado por `/dashboard/personal-documents`) |
...
| `getDocumentFileUrlRequest` | `GET /api/v1/document/file/:id` |

**`app/dashboard/documents/create/_requests.ts`**

| Función | Endpoint |
|---|---|
| `createDocumentSignaturesRequest` | `POST /api/v1/documents/signatures` (multipart: `file` + `documentData` + `collaboratorsdata` + `requiresDifferentSignatures`) — reemplazó el patrón viejo `POST /api/v1/document` + `submit-for-authorization` |
...
| Función | Endpoint |
|---|---|
| `getDocumentDetailRequest` | `GET /api/v1/document/:id` |
| `signDocumentRequest` | `PATCH /api/v1/document/:id/sign` |
| `rejectDocumentRequest` | `PATCH /api/v1/document/:id/reject` |
| `requestCancellationRequest` / `confirmCancellationRequest` | `PATCH /api/v1/document/:id/submit-for-cancellation` \| `/confirm-cancellation` |
| `requestVerificationCodeRequest` / `verifyCodeRequest` | `POST /api/v1/document/:id/verification-codes` \| `/verify` — 2FA de firma |
| `linkCollaboratorRequest` | `PATCH /api/v1/document/:id/link-collaborator` — usado por `/access-document` |
...
| `updateIneFileRequest` / `updateSignatureFileRequest` | `PATCH /api/v1/signature/:id` |
| `deleteIneFileRequest` / `deleteSignatureFileRequest` | `DELETE /api/v1/signature/:id/official-file` \| `/signature-image` |
...
| `verifyResetCodeRequest` | `POST /api/v1/auth/verify-reset-code` |
...
- **De los dos headers que manda `lib/axios.ts`, solo `X-Account-Id` tiene efecto, y solo en documentos (crear + listar) y en la invitación/gestión de miembros**: cambiar de cuenta en el switcher ya filtra `/api/v1/document` de verdad, y las rutas de organización ya validan contra la cuenta activa — ver README de `signature-server`. `X-Organization-Id` se manda igual en cada request (cuando la cuenta activa es una organización) pero **ningún endpoint del backend lo lee todavía**. El resto de recursos (detalle de documento, firma/rechazo/cancelación) tampoco distinguen cuenta activa; cambiar de cuenta ahí sigue sin efecto en el backend. (`GET /api/v1/user` para elegir firmantes/espectadores ya no aplica: `/documents/create` dejó de usar ese picker, ver "Resuelto en esta ronda" de la historia de carga de documentos.)
...
- **Sin cambios de código de este lado**: `lib/axios.ts` ya mandaba `X-Account-Id`/`X-Organization-Id` desde que se construyó el interceptor (parte del trabajo de Zustand). Lo que cambió fue el backend: ahora valida el header contra una membresía real y filtra `GET /api/v1/document` por la cuenta activa — ver README de `signature-server` para el detalle (nueva columna `accountId` en documentos, con backfill a la cuenta personal del creador para los documentos existentes).
