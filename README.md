...
`roleId`/`status` en `accountsList` (`roleId: string | null`, `status: 'ACTIVE' | 'INACTIVE'`) ya reflejan el rol/vigencia real de la membresía — `toAccountListEntry` los mapea desde `role`/`isActive` del backend (ver Pendientes → Resuelto más abajo).
...
### Resuelto en esta ronda (roleId/status reales en accountsList)
- **`toAccountListEntry` ya mapea `roleId`/`status` reales**: `AccountData` (`lib/api/accounts.ts`) gana `role`/`isActive`, que el backend ahora manda de verdad (`GET /api/v1/accounts/me` y `POST /api/v1/organizations` — ver README de `signature-server`). `roleId` toma el primer rol de la membresía (`role?.[0] ?? null`) y `status` es `'ACTIVE'`/`'INACTIVE'` según `isActive`. El switcher todavía no diferencia visualmente por rol/estatus en la UI (nadie lo pidió aún), pero el dato ya está disponible en `accountsList` para cuando se necesite.
- **Ya no hace falta UI para asignar el `roleId` de una organización recién creada**: el backend decidió asignar `OWNER` de inmediato al creador (ver README de `signature-server`), en vez de dejarlo en `NULL` esperando un flujo de asignación que nunca se construyó. `useCreateOrganization` no necesitó cambios — ya usaba `toAccountListEntry(account)` sobre la respuesta real del backend.
