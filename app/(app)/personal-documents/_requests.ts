...
export async function updateIneFileRequest(signatureId: string, file: File): Promise<void> {
  const formData = new FormData();
  formData.append('officialFile', file);
  await apiClient.patch(`/signature/${signatureId}`, formData);
}

export async function updateSignatureFileRequest(signatureId: string, file: File): Promise<void> {
  const formData = new FormData();
...
export async function deleteSignatureFileRequest(signatureId: string): Promise<void> {
